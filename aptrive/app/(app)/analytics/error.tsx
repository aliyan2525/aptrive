...
export default function AnalyticsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <section className="container-aptrive flex min-h-[60vh] flex-col items-center justify-center py-16 text-center">
      <div className="eyebrow">Analytics unavailable</div>
      <h1 className="font-display mt-3 text-3xl font-semibold tracking-tight text-fg sm:text-4xl">Your progress view needs another moment.</h1>
      <p className="mt-3 max-w-md text-sm leading-6 text-muted">Your account is safe. Try loading analytics again, or start a practice session while we rebuild the view.</p>
