...
export default function PremiumFeatureCard({ title, body, icon, delay = 0, color }: PremiumFeatureCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`group relative h-full rounded-3xl border border-white/5 bg-panel p-8 transition-all duration-500 hover:-translate-y-2 hover:bg-white/[0.02] ${theme.glow}`}
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="absolute inset-0 rounded-3xl border border-transparent transition-colors duration-500 group-hover:border-white/10 pointer-events-none" />
      </div>
    </motion.div>
  );
}
