    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/35 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="subscription-modal-title" className="max-h-[92dvh] w-full overflow-y-auto rounded-t-[2rem] border border-white/90 bg-[#f8faff]/95 p-5 shadow-[0_28px_100px_rgba(25,35,85,0.25)] backdrop-blur-2xl sm:max-w-3xl sm:rounded-[2rem] sm:p-7">
        <div className="flex items-start justify-between gap-4 border-b border-line pb-5">
          ...
          <button ref={closeButtonRef} type="button" onClick={onClose} aria-label="Close subscription manager" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-white/60 bg-white/70 backdrop-blur-lg shadow-sm text-muted transition hover:border-violet-300 hover:text-fg"><X className="h-5 w-5" aria-hidden="true" /></button>
        </div>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-3">
          {plans.map((plan) => {
            ...
            return <button key={plan.id} type="button" onClick={() => setSelectedPlan(plan.id)} aria-pressed={active} className={`relative rounded-2xl border p-4 text-left transition ${active ? "border-violet-500 bg-violet-50/80 shadow-[0_12px_32px_rgba(111,69,255,0.12)]" : "border-white/60 bg-white/70 backdrop-blur-lg shadow-[0_4px_24px_rgba(0,0,0,0.02)] hover:border-violet-300"}`}>
