...
export default function BillingHistorySection({ invoices = [] }: { invoices?: BillingInvoice[] }) {
  return (
    <PanelShell>
      ...
      {invoices.length > 0 ? (
        <div className="premium-shell mt-5 overflow-hidden rounded-2xl border border-white/80 bg-white/60 backdrop-blur-xl">
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-violet-200 bg-[linear-gradient(135deg,rgba(247,245,255,0.8),rgba(239,252,249,0.7))] p-5 sm:p-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-white/60 bg-white/70 backdrop-blur-lg px-3 text-xs font-semibold text-muted"><Download className="h-4 w-4" aria-hidden="true" /> No downloads</span>
          </div>
        </div>
      )}
